...
public class BasicBoxExteriorSchema extends TARDISExteriorSchema<BasicBoxModel> {
    ...
    private static final ResourceLocation LOCATION = new ResourceLocation(AIT.MOD_ID, "textures/exteriors/basic_exterior.png");
    private static final ResourceLocation BASIC_LM_LOCATION = new ResourceLocation(AIT.MOD_ID, "textures/exteriors/basic_exterior_emission.png");

    private static final ResourceLocation CHRISTMAS_LOCATION = new ResourceLocation(AIT.MOD_ID, "textures/exteriors/holidays/basic_exterior_christmas.png");
    private static final ResourceLocation SNOW_LOCATION = new ResourceLocation(AIT.MOD_ID, "textures/exteriors/biomes/snow/basic_exterior_snow.png");
    private static final ResourceLocation SAND_LOCATION = new ResourceLocation(AIT.MOD_ID, "textures/exteriors/biomes/sand/basic_exterior_sand.png");
    ...
    public BasicBoxModel render(RenderInfo info, TARDISTileEntity tile, BasicBoxModel model) {
        ...
        info.stack.mulPose(Vector3f.YP.rotationDegrees(tile.getBlockState().getValue(BlockStateProperties.HORIZONTAL_FACING).toYRot()));
        model.render(tile, info.stack, info.buffer.getBuffer(AITRenderTypes.tardisLightmap(BASIC_LM_LOCATION, false)), info.light, info.overlay, 1, 1, 1, 1);
        info.stack.popPose();
    ...
    private void smithMintPosterText(RenderInfo info) {
        ...
        fontRenderer.drawInBatch(irp, -5, 5, 16777215, false, info.stack.last().pose(), info.buffer, false, 0, info.light);
        info.stack.popPose();
        info.stack.pushPose();
