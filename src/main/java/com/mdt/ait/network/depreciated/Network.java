...
public class Network {
    ...
    private static final SimpleChannel NETWORK_CHANNEL = NetworkRegistry.newSimpleChannel(
            new ResourceLocation(AIT.MOD_ID, "main_channel"),
            () -> PROTOCOL_VERSION,
    ...
    public static <PACKET extends Consumer<NetworkEvent.Context>> void registerCodecPacket(
            int id, SimpleChannel channel, Codec<PACKET> codec, PACKET defaultPacket) {

        final BiConsumer<PACKET, PacketBuffer> encoder =
                (packet, buffer) -> codec.encodeStart(NBTDynamicOps.INSTANCE, packet)
                        .result()
                        .ifPresent(nbt -> buffer.writeNbt((CompoundNBT) nbt));
        final Function<PacketBuffer, PACKET> decoder = buffer ->
                codec.parse(NBTDynamicOps.INSTANCE, buffer.readNbt()).result().orElse(defaultPacket);
        ...
        // NETWORK_CHANNEL.registerMessage(id, MonitorExteriorChangePacket.class,
        // MonitorExteriorChangePacket::encode, MonitorExteriorChangePacket::decode,
        // MonitorExteriorChangePacket::handle);
    }
