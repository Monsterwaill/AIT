...
public class CyberCavalryEntity extends MonsterEntity {
    private static final DataParameter<Integer> DATA_SPECIAL_TYPE_ID =
            EntityDataManager.defineId(CyberCavalryEntity.class, DataSerializers.INT);
    ...
    protected void addBehaviourGoals() {
        ...
        this.targetSelector.addGoal(
                ...
                new NearestAttackableTargetGoal<>(
                        this, TurtleEntity.class, 10, true, false, TurtleEntity.BABY_ON_LAND_SELECTOR));
    }
    ...
    protected void handleAttributes(float pDifficulty) {
        this.getAttribute(Attributes.KNOCKBACK_RESISTANCE)
                .addPermanentModifier(new AttributeModifier(
                        "Random spawn bonus",
                        this.random.nextDouble() * (double) 0.05F,
                        AttributeModifier.Operation.ADDITION));
        double d0 = this.random.nextDouble() * 1.5D * (double) pDifficulty;
        if (d0 > 1.0D) {
            this.getAttribute(Attributes.FOLLOW_RANGE)
                    .addPermanentModifier(new AttributeModifier(
                            "Random zombie-spawn bonus", d0, AttributeModifier.Operation.MULTIPLY_TOTAL));
        }
    ...
    class AttackTurtleEggGoal extends BreakBlockGoal {
        ...
        public void playDestroyProgressSound(IWorld pLevel, BlockPos pPos) {
            pLevel.playSound(
                    ...
                    0.5F,
                    0.9F + CyberCavalryEntity.this.random.nextFloat() * 0.2F);
        }
