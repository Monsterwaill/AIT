...
public class RecordPlayerTile extends TileEntity implements ITickableTileEntity {
    ...
    public ActionResultType useOn(
        ...
        Item iteminmainhand =
                playerEntity.getItemBySlot(EquipmentSlotType.MAINHAND).getItem();
        System.out.println(iteminmainhand);
        if (!playerEntity.isCrouching()
                && hand == Hand.MAIN_HAND
                && !world.isClientSide
                && iteminmainhand == Items.AIR
                && !isHasRecord) {
            playerState = getNextPlayerState();
            syncToClient();
        }
        if ((playerState == EnumRecordPlayerState.OPEN || playerState == EnumRecordPlayerState.PLAYING)
