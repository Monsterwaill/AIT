...
public class ARSRemoverTile extends TileEntity implements ITickableTileEntity {
    ...
    /*
     * Displays text on the screen with the number and some info below it eg: 2 room deletion
     * incoming 1 Second = 20 ticks.
     */
    public void cloisterCountdown(
            World world,
            Direction direction,
            int timer,
            BlockPos pos,
        ...
        if (this.countdown) {
            if (ticks >= 0) {
                ...
                if (ticks % 40 == 0) {
                    ...
                    player.sendMessage(
                            new TranslationTextComponent(this.seconds + " " + this.message)
