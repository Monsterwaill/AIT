        ...
        if (tardisID != null) {
            if (this.getLevel() != null) {
                if (!this.getLevel().isClientSide()) {
                    /*
                     * Tardis tardis = AIT.tardisManager.getTardis(this.tardisID); ServerWorld world
                     * = AIT.server.getExteriorLevel(tardis.exterior_dimension); ServerWorld
                     * tardisWorld = AIT.server.getExteriorLevel(AITDimensions.TARDIS_DIMENSION);
                     * assert world != null; assert tardisWorld != null; if (leverState ==
                     * EnumLeverState.ACTIVE) { if
                     * (AIT.tardisManager.doesTardisHaveATargetLocation(this.tardisID)) {
                     * tardisWorld.playSound(null, tardis.centerPosition,
                     * AITSounds.TARDIS_TAKEOFF.get(), SoundCategory.MASTER, 7, 1);
                     * this.dematTransit =
                     * AIT.tardisManager.moveTardisToTargetLocation(this.tardisID); if
                     * (this.dematTransit.leverPos != this.worldPosition) {
                     * this.dematTransit.leverPos = getBlockPos(); } } else { leverState =
                     * EnumLeverState.DEACTIVE; } }
                     */
                }
            }
        }
                ...
                if (tardisID != null) {
                    if (this.getLevel() != null) {
                        if (!this.getLevel().isClientSide()) {
                            ...
                            PlayerEntity playerEntity = serverWorld.getNearestPlayer(
                                    worldPosition.getX(), worldPosition.getY(), worldPosition.getZ(), 60, false);
                ...
                if (this.dematTransit.readyForDemat) {
                    ...
                    if (playerEntity != null) {
                        ...
                        playerEntity.displayClientMessage(
                                new TranslationTextComponent("Flight remaining: "
                                                + (flightTicks * 100) / this.dematTransit.getFlightTicks()
                                                + "%")
                    ...
                    if (flightTicks == this.dematTransit.getFlightTicks()) {
                        ...
                        if (playerEntity != null) {
                            playerEntity.sendMessage(
                                    new TranslationTextComponent("TARDIS is ready for rematerialisation.")
                                            .setStyle(Style.EMPTY
    ...
    public ActionResultType useOn(
        ...
        if (!world.isClientSide && world.dimension() == AITDimensions.TARDIS_DIMENSION) {
            ...
            if (block instanceof TardisLeverBlock && hand == Hand.MAIN_HAND) {
                ...
                if (leverState == EnumLeverState.ACTIVE) {
                    ...
                    if (flightTicks <= 5) {
                        if (this.dematTransit == null) {
                            ...
                            playerEntity.sendMessage(
                                    new TranslationTextComponent("Dematerialising...")
                    ...
                    /*
                     * BasicInteriorDoorTile basicInteriorDoorTile = (BasicInteriorDoorTile)
                     * tardisWorld.getBlockEntity(interiorDoorPos); if (basicInteriorDoorTile !=
                     * null) { basicInteriorDoorTile.setLockedState(true, EnumDoorState.CLOSED);
                     * tardisWorld.playSound(null, interiorDoorPos, AITSounds.TARDIS_LOCK.get(),
                     * SoundCategory.MASTER, 7, 1); sync(); }
                     */
                    // }
                    if (this.dematTransit != null) {
                        if (flightTicks >= this.dematTransit.getFlightTicks()) {
                            if (leverState == EnumLeverState.ACTIVE) {
                                ...
                                syncToClient();
                                /*
                                 * AIT.tardisManager.rematerialize = true; BlockPos exteriorPos =
                                 * tardis.exteriorPosition; assert tardisWorld != null;
                                 * TardisTileEntity tardisTileEntity = (TardisTileEntity)
                                 * world.getBlockEntity(exteriorPos); assert tardisTileEntity !=
                                 * null; tardisWorld.playSound(null, tardis.centerPosition,
                                 * AITSounds.TARDIS_LANDING.get(), SoundCategory.MASTER, 7, 1); if
                                 * (interiorDoorPos != null) { BasicInteriorDoorTile
                                 * basicInteriorDoorTile = (BasicInteriorDoorTile)
                                 * tardisWorld.getBlockEntity(interiorDoorPos); if
                                 * (basicInteriorDoorTile != null) {
                                 * basicInteriorDoorTile.setLockedState(false,
                                 * EnumDoorState.CLOSED); tardisWorld.playSound(null,
                                 * interiorDoorPos, AITSounds.TARDIS_LOCK.get(),
                                 * SoundCategory.MASTER, 7, 1); flightTicks = 0; sync(); } }
                                 */
                            }
                ...
                } else if (leverState == EnumLeverState.DEACTIVE
                        && this.dematTransit != null
                        && this.dematTransit.isReadyForRemat) {
                    // tardisWorld.playSound(null, tardis.centerPosition,
                    // AITSounds.TARDIS_LANDING.get(), SoundCategory.MASTER, 7, 1);
                    this.dematTransit.landTardisPart2();
                    playerEntity.sendMessage(
                            new TranslationTextComponent("Rematerialising...")
