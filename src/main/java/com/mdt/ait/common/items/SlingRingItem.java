...
public class SlingRingItem extends Item {
    ...
    public ActionResultType useOn(ItemUseContext pContext) {
        Block block =
                pContext.getLevel().getBlockState(pContext.getClickedPos()).getBlock();
        ...
        if (!pContext.getLevel().isClientSide()) {
            if (portal != null) {
                ...
                if (!(block instanceof AirBlock)) {
                    float yRotation = Objects.requireNonNull(pContext.getPlayer())
                            .getDirection()
                            .toYRot();
                    portal.setOrientationAndSize(new Vector3d(1.0D, 0.0D, 0.0D), new Vector3d(0.0D, 1.0D, 0.0D), 2, 2);
                    PortalManipulation.rotatePortalBody(portal, new Quaternion(Vector3f.YP, yRotation, true));
                    portal.setOriginPos(new Vector3d(
                            pContext.getClickedPos().getX() + 0.5D,
                    ...
                    PortalManipulation.setPortalTransformation(
                            ...
                            new Vector3d(
                                    ...
                                    pContext.getClickedPos().getY(),
                                    pContext.getClickedPos().getZ() + 8D),
                            new Quaternion(Vector3f.YP, -yRotation, true),
                            1d);
                    this.portal2.setDestination(new Vector3d(
                            pContext.getClickedPos().getX() + 0.5D,
                            pContext.getClickedPos().getY(),
                            pContext.getClickedPos().getZ() + 1D));
