...
public class SonicItem extends Item {
    ...
    public void appendHoverText(
            ItemStack pStack, @Nullable World pWorldIn, List<ITextComponent> pTooltip, ITooltipFlag pFlagIn) {
    ...
    public ActionResultType useOn(ItemUseContext context) {
        ...
        if (block instanceof TNTBlock) {
            ...
            world.playSound(
                    null, tnt.getX(), tnt.getY(), tnt.getZ(), SoundEvents.TNT_PRIMED, SoundCategory.BLOCKS, 1.0F, 1.0F);
            return ActionResultType.sidedSuccess(world.isClientSide());
        }
        if (block instanceof TARDISBlock) {
            ...
            if (tileEntity instanceof TARDISTileEntity) {
                if (!world.isClientSide) {
                    // ((TARDISTileEntity) tileEntity).useOnTardis(context, blockpos, blockstate,
                    // block); FIXME: dis
                }
            }
            return ActionResultType.sidedSuccess(world.isClientSide());
        ...
        if (block instanceof DoorBlock) {
            ...
            world.levelEvent(
                    playerentity,
                    blockstate.getValue(OPEN)
                            ? blockstate.getMaterial() == Material.METAL ? 1005 : 1006
                            : blockstate.getMaterial() == Material.METAL ? 1011 : 1012,
                    blockpos,
                    0);
        }
        ...
        if (block instanceof RedstoneWireBlock) {
            ...
            if (i == 15) {
                world.setBlock(blockpos, blockstate.setValue(POWER, i = 0), 2);
            } else {
                world.setBlock(blockpos, blockstate.setValue(POWER, i + 1), 2);
            }
        }
