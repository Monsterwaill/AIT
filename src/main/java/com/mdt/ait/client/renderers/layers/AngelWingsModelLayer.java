...
public class AngelWingsModelLayer<T extends AbstractClientPlayerEntity, M extends EntityModel<T>>
        extends LayerRenderer<T, M> {

    public static final ResourceLocation WHITE_WINGS_LOCATION =
            new ResourceLocation(AIT.MOD_ID, "textures/cosmetics/white_angel_wings.png");
    ...
    private final float wingAlpha = 1;
    /*
     * public float xWings0; public float xWings; public float yWings0; public float yWings; public
     * float zWings0; public float zWings;
     */
    ...
    public void render(
        ...
        if (item == AITItems.ANGEL_WINGS.get()) {
            ...
            // this.wingAlpha = 0f;
            // }
            // angelWings.renderToBuffer(pMatrixStack, vertexBuffer, pPackedLight,
            // OverlayTexture.NO_OVERLAY, 1, 1, 1, this.wingAlpha);
    ...
    /*
     * double d0 = MathHelper.lerp((double)pPartialTicks, this.xWings0, this.xWings) -
     * MathHelper.lerp((double)pPartialTicks, pLivingEntity.xo, pLivingEntity.getX()); double d1 =
     * MathHelper.lerp((double)pPartialTicks, this.yWings0, this.yWings) -
     * MathHelper.lerp((double)pPartialTicks, pLivingEntity.yo, pLivingEntity.getY()); double d2 =
     * MathHelper.lerp((double)pPartialTicks, this.zWings0, this.zWings) -
     * MathHelper.lerp((double)pPartialTicks, pLivingEntity.zo, pLivingEntity.getZ()); float f =
     * pLivingEntity.yBodyRotO + (pLivingEntity.yBodyRot - pLivingEntity.yBodyRotO); double d3 =
     * (double)MathHelper.sin(f * ((float)Math.PI / 180F)); double d4 = (double)(-MathHelper.cos(f *
     * ((float)Math.PI / 180F))); float f1 = (float)d1 * 10.0F; f1 = MathHelper.clamp(f1, -6.0F,
     * 32.0F); float f2 = (float)(d0 * d3 + d2 * d4) * 100.0F; f2 = MathHelper.clamp(f2, 0.0F,
     * 150.0F); float f3 = (float)(d0 * d4 - d2 * d3) * 100.0F; f3 = MathHelper.clamp(f3, -20.0F,
     * 20.0F); if (f2 < 0.0F) { f2 = 0.0F; }
     *
     * float f4 = MathHelper.lerp(pPartialTicks, pLivingEntity.oBob, pLivingEntity.bob); f1 = f1 +
     * MathHelper.sin(MathHelper.lerp(pPartialTicks, pLivingEntity.walkDistO,
     * pLivingEntity.walkDist) * 6.0F) * 32.0F * f4; if (pLivingEntity.isCrouching()) { f1 += 25.0F;
     * }
     *
