...
public class DimensionSwitchControlTile extends TileEntity implements ITickableTileEntity {
        ...
        if (this.tardisID != null) {
            if (this.getLevel() != null) {
                if (!this.getLevel().isClientSide()) {
                     ...
                     * (!canChangeDim) { if (newDimension == null) { newDimension =
                     * tardis.targetDimension; if
                     * (ExteriorWorld.getBlockEntity(tardis.exteriorPosition) != null) {
                     * if(tardis.exterior_dimension != tardis.targetDimension) { newDimension =
                     * tardis.exterior_dimension; } else { if (tardis.exterior_dimension ==
                     * AIT.server.overworld().dimension()) { currentdimensionstate =
                     * EnumDimensionControlState.EARTH; sync(); } if (tardis.exterior_dimension ==
                     * AIT.server.getExteriorLevel(World.NETHER).dimension()) {
                     * currentdimensionstate = EnumDimensionControlState.NETHER; sync(); } if
                     * (tardis.exterior_dimension ==
                     * AIT.server.getExteriorLevel(World.END).dimension()) { currentdimensionstate =
                     * EnumDimensionControlState.END; sync(); } if (tardis.exterior_dimension ==
                     * AIT.server.getExteriorLevel(AITDimensions.GALLIFREY).dimension()) {
                     * currentdimensionstate = EnumDimensionControlState.GALLIFREY; sync(); } if
                     * (tardis.exterior_dimension ==
                     * AIT.server.getExteriorLevel(AITDimensions.MONDAS).dimension()) {
                     * currentdimensionstate = EnumDimensionControlState.MONDAS; sync(); } } } } }
            ...
            if (this.getLevel() != null) {
                if (!this.getLevel().isClientSide()) {
                    if (block instanceof DimensionSwitchControlBlock && hand == Hand.MAIN_HAND) {
                        ...
                        // "IS THIS
                        // WORKING?!").setStyle(Style.EMPTY.withColor(TextFormatting.YELLOW)),
                        // UUID.randomUUID());
    ...
    public void onPlace() {
        /*
         * Tardis tardis = AIT.tardisManager.getTardis(tardisID); newDimension =
         * tardis.exterior_dimension; if(tardis.exterior_dimension ==
         * AIT.server.overworld().dimension()) { currentdimensionstate =
         * EnumDimensionControlState.EARTH; } if(tardis.exterior_dimension ==
         * AIT.server.getExteriorLevel(World.NETHER).dimension()) { currentdimensionstate =
         * EnumDimensionControlState.NETHER; } if(tardis.exterior_dimension ==
         * AIT.server.getExteriorLevel(World.END).dimension()) { currentdimensionstate =
         * EnumDimensionControlState.END; } if(tardis.exterior_dimension ==
         * AIT.server.getExteriorLevel(AITDimensions.GALLIFREY).dimension()) { currentdimensionstate
         * = EnumDimensionControlState.GALLIFREY; } if(tardis.exterior_dimension ==
         * AIT.server.getExteriorLevel(AITDimensions.MONDAS).dimension()) { currentdimensionstate =
         * EnumDimensionControlState.MONDAS; } changeDimensionFromControl(); sync();
         */
    }
