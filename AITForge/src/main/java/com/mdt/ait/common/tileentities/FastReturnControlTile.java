        ...
        if (block instanceof FastReturnControlBlock && hand == Hand.MAIN_HAND) {
            ...
            if (this.tardisID != null) {
                if (!level.isClientSide) {
                     ...
                     * AIT.tardisManager.setTardisTargetBlockPos(tardisID,
                     * tardis.previousExteriorPos); tardis.targetFacingDirection =
                     * tardis.previousDirection; tardis.targetDimension = tardis.previous_dimension;
                     * if (playerEntity != null) { playerEntity.sendMessage(new
                     * TranslationTextComponent("Set Coords to " + tardis.previousExteriorPos.getX()
                     * + " " + tardis.previousExteriorPos.getY() + " " +
                     * tardis.previousExteriorPos.getZ()).setStyle(Style.EMPTY.withColor(
                     * TextFormatting.YELLOW).withItalic(true)), UUID.randomUUID()); }
                     */
                }
            }
        }
